...
if ( ! empty( $_POST ) ) {
    if (  isset( $_POST['id'] ) ) {
        ...
        $row = pg_fetch_row($result);
          ...
          <input type=\"text\" class=\"form-control\" name=\"hotel_id\" placeholder=\"Hotel Id\" value=".$row[0]." required hidden>
...
if ( ! empty( $_POST ) ) {
    if ( isset( $_POST["hotel_id"] ) ) { 
        $query = 'delete from public.Hotel where hotel_id='.$_POST["hotel_id"];
        $result = pg_query($query) ;
        print_r($query);
